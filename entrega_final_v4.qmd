...
nuevos_regresores_m2 <- as.matrix(xreg_prediccion_m2[74:76,])
...
autoplot(cantidad_clientes_ts_completa) +
  ...
  labs(title = "Predicción del Modelo SARIMA(2,1,0)(1,1,0)",
...
Obtenemos algunas medidas de los errores de predicción sobre el conjunto de testeo. Para ello los errores son tomados como la diferencia en cada punto de la serie entre el conjunto de testeo (los valores reales) y los valores predichos, es decir, $(x - \hat{x})$.
...
Un ME positivo implica que el modelo en promedio subestima y si observamos el MPE vemos con qué porcentaje. En el caso de las predicciones a un pas sobre el conjunto de entrenamiento, los errores se compensan lo suficiente generando que dicha subestimación sea considerablemente pequeña, con un PME de solo el 7,6%. Por otro lado, para los errores de predicción sobre el conjunto de testeo, la subestimación es relativamente grande, en promedio de un 66%.

El MAE presenta un error en términos absolutos que, para ambos casos, en comparación con el rango de valores de la serie no parece ser tan alto. Sin embargo, el MAPE, el cual muestra errores en valores absolutos, pero a diferencia del MAE, en términos relativos, es considerablemente alto. El modelo comete en promedio un 44% de error relativo respecto al valor real para el caso del set de training y un 66% para el caso del set de testeo.

Es preciso señalar que los valores de ME y MAE son prácticamente los mismos para el set de testeo, esto solo se puede dar si en todos los puntos predichos se subestimó el valor real, como se ve en gráficos anteriores. 

El RMSE, el cual penaliza errores grandes, si bien es más alto que el MAE para ambos casos, en proporción a la media de la serie representa solo un 0,6% y un 1% respectivamente. Es decir que no se presentan errores extremadamente grandes en las predicciones.

El MASE es una métrica que compara los errores de predicción con los de un modelo naive de 1 lag. En este caso nuestro modelo predice ampliamente mejor que uno naive, ya que los valores 7,2%, para el set de training, y 13,5%, para el set de testeo, son extremadamente bajos.

Por su ado, el ACF1, una medida de la correlación entre errores a un rezago, muestra que existe poca correlación entre los errores del set de training, mientras que para el set de testeo parece existir sierta correlación negativa.
...
Para las predicciones a un paso sobre el set de training, los errores en este período presentan métricas muy similares a las de las predicciones del período anterior. Solo el ME, en conjunto con el MPE en términos relativos, disminuyó notablemente. Esto implica que los errores en promedio se compensan aún más.
...
Según el ME y el MPE, se subestims ampliamente los valores de la serie empírica. Esto se puede observar también en gráficos anteriores.

El RMSE y el MAE también aumentaron notablemente, si bien en comparación con la escala no parecen ser tan altos y en proporción a la media solo representan un 8%. 

Analizándolo en términos relativos, el MAPE implica un error de predicción muy alto.

Por su parte, el MASE, al ser mayor que 1, sugiere que un modelo naive predice mejor que el modelo propuesto.

El ACF1 mejora en comparación al período anterir, aún así, sigue existiendo una correlación negativa entre los errores.
...
Para este último período de predicción, las métricas para el conjunto de training también son muy parecidas a los anteriores dos períodos.

Por otro lado, para el set de testeo, las métricas son en general mejores que las del anterior período tomado, pero relativamente peores que las del primero. Esto es de esperarse nuevamente debido al horizonte de predicción.

La subestimación según el ME y el MPE sigue siendo alta.

El RMSE no es tan grande respecto a la escala de la serie, es decir que no parece haber errores inusualmente altos.

Segpun el MASE, el modelo propuesto predice mejor en comparación con un modelo naive.

Por último, el MAE, MAPE, ACF1 y Theil's U sugieren erores de predicción bastante altos.
...
- Comentario sobre las medidas de predicción.

- Se logra predecir con éxito a un bajo nivel de confianza dos de tres valores que tomó la serie en 2025: los asociados a los meses de Enero y Marzo.
