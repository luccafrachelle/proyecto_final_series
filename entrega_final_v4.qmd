...
acc1 <- round(accuracy(Predicciones_Modelo_2_Tabla_ICs, test_clientes), 4)[1,1:7]
acc1 <- as.data.frame(t(acc1))  # transponer para que las métricas queden como columnas
kable(acc1, caption = r"(Métricas de Predicción - SARIMA$(2,1,0)(1,1,0)$)") 
```


Un ME positivo implica que el modelo en promedio subestima. Sin embargo, si observamos el MPE, vemos que los errores se compensan lo suficiente como para que dicha subestimación sea considerablemente pequeña, ya que un PME del 7,6% es relativamente bajo.

El MAE presenta un error en términos absolutos que en comparación con el rango de valores de la serie no parece tan alto. Sin embargo, el MAPE, el cual muestra errores en valores absolutos, pero a diferencia del MAE, en términos relativos, presenta es considerablemente alto. El modelo comete en promedio un 44% de error relativo respecto al valor real.

EL RMSE, el cual penaliza errores grandes, si bien es más alto que el MAE, en proporción a la media de la serie representa solo un 0,6%. Es decir que no se presentan errores extremadamente altos en las predicciones.

El MASE es una métrica que compara los errores de predicción con los de un modelo naive de 1 lag. En este caso nuestro modelo predice ampliamente mejor que uno naive, ya que un valor del 7,2% es extremadamente bajo.

Por último, el ACF1, una medida de la correlación entre errores a un rezago, muestra que existe poca correlación entre los errores, lo cual coincide con los tests realizados anteriormente.
...
```{r}
acc2 <- round(accuracy(Pred_Final_Test_m2, test_clientes), 4)[1,1:7]
acc2 <- as.data.frame(t(acc2))  # transponer para que las métricas queden como columnas
kable(acc2, caption = r"(Métricas de Predicción - SARIMA$(2,1,0)(1,1,0)$)") 
```

Los errores de predicción para este período presentan métricas muy similares a las de las predicciones para únicamente los 3 primeros meses de 2025. Solo el ME, en conjunto con el MPE en términos relativos, disminuyó notablemente. Esto implica que los errores en promedio se compensan.
...
```{r}
acc3 <- round(accuracy(Pred_Final_Test_m2_2, test_clientes), 4)[1,1:7]
acc3 <- as.data.frame(t(acc3))  # transponer para que las métricas queden como columnas
kable(acc3, caption = r"(Métricas de Predicción - SARIMA$(2,1,0)(1,1,0)$)") 
```

Para este último período de predicción, las métricas sobre los errores de predicción también son muy parecidas a los anteriores dos períodos. Se aprecia una leve mejora en el RMSE, la cual podría deberse a que no tomamos en cuenta el efecto de Junio de 2024 dentro de la muestra de entrenamiento para realizar las predicciones.
