...
A continuación, se presentan los dos modelos SARIMA finalistas seleccionados para la serie temporal. Cada uno ha sido ajustado con intervención de valores atípicos y sometido a un riguroso proceso de diagnóstico.

## Modelo 1: SARIMA(6,1,0)(1,1,0)[3] con Intervención de Atípicos

El primer modelo explora una estacionalidad trimestral (período 3).
...
Se realiza una predicción para los próximos 3 meses.
...
residuos_sq_acf_m2 <- ggAcf(residuos_m2_sq, lag.max=24, type = "correlation") + labs(title = "FAC del Cuadrado de los Residuos del Modelo 2")
residuos_sq_pacf_m2 <- ggAcf(residuos_m2_sq, lag.max=24, type = "partial") + labs(title = "FACP del Cuadrado de los Residuos del Modelo 2")
