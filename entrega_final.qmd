...
Un ME (Mean Error o Error Medio) positivo implica que el modelo en promedio subestima. Es posible ver con qué porcentaje al observar el MPE (Mean Percentage Error o Error Medio Porcentual). En el caso de las predicciones a un paso sobre el conjunto de entrenamiento^[Se recuerda que en este caso el set de entrenamiento está compuesto por las observaciones hasta diciembre de 2024, mientras que el de prueba o test está integrado por las tres observaciones disponibles para 2025.], los errores se compensan lo suficiente generando que dicha subestimación sea considerablemente pequeña, con un MPE de solo el 0,076%. Por otro lado, para los errores de predicción sobre el conjunto de testeo, la subestimación es mayor, en promedio de un 0,66%, lo que está explicado por la predicción referida a Febrero de 2025.

El MAE (Mean Absolute Error o Error Absoluto Medio) presenta un error en términos absolutos que, para ambos casos, en comparación con el rango de valores de la serie, no parece ser tan alto. El MAPE (Mean Absolute Percentage Error o Error Medio Absoluto Porcentual), el cual muestra dichos errores en términos relativos, es considerablemente muestra que el modelo comete en promedio un 0,44% de error relativo respecto al valor real para el caso del set de entrenamiento y un 0,66% para el caso de los tres meses de 2025.
...
El MASE (Mean Absolute Scaled Error o Error Medio Absoluto Escalado) es una métrica que compara el MAE de la predicción con el MAE que se obtiene al utilizar un modelo naive de un lag. En este caso el modelo planteado predice ampliamente mejor que uno naive, ya que los valores 0.072, para el set de training, y 0,13, para el set de testeo, son extremadamente bajos.
...
Analizándolo en términos relativos, el MAPE implica un error de predicción alto.
